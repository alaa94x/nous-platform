  ...
  return (
    <section id="contact" aria-label={dictionary.contact.aria} lang={locale} dir={dictionary.direction} className="convergence">
      ...
      <div className="convergence-shell">
        ...
        <div className="convergence-layout">
          <div className="convergence-copy" data-reveal={isAr ? 'rtl' : 'copy'}>
            <span>{isAr ? 'نبدأ بما تحتاجه' : 'Start with what you need'}</span>
            <h2>{heading}</h2>
          ...
          <div className="convergence-action" data-reveal="copy">
            <Link href={isAr ? '/ar/contact' : '/contact'} className="convergence-link pressable">
              <span>{cta ?? dictionary.contact.cta}</span><ArrowUpRightIcon size={20} />
            </Link>
            <div>
      ...
      <style>{`
        ...
        .convergence-shell{position:relative;z-index:2;width:min(100%,1480px);margin:0 auto}.convergence-meta{display:grid;grid-template-columns:auto minmax(80px,1fr) auto;align-items:center;gap:16px;font-family:var(--font-mono);font-size:9px;font-weight:700;letter-spacing:.16em;text-transform:uppercase;color:var(--light-micro)}.convergence-meta i{height:1px;background:rgba(7,17,14,.24)}
        ...
        @media(max-width:780px){.convergence{min-height:0;padding:76px 20px 108px}.convergence-meta{display:none}.convergence-layout{grid-template-columns:1fr;gap:38px;margin-top:0}.convergence-copy h2{font-size:clamp(38px,11vw,52px);line-height:1}.convergence-copy p{margin-top:20px;font-size:16px}.convergence-link{min-height:58px}.convergence-action>div{flex-direction:column;gap:7px}.convergence-field{opacity:.32}}
